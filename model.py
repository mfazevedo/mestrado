    ...
    def custo(self):
        ...
        for rota in self.rotas:
            for i in range(len(rota.clientes) - 1):
                destino = rota.clientes[i + 1]
                cliente = rota.clientes[i]
...
class Cliente(object):
    ...
    def mais_proximo(self, clientes):
        ...
        if not (self.id == clientes[0].id):
            custo = self.distancia(
                clientes[0])  # Toma como menor distância inicial a distância do depósito até o primeiro cliente listado
            retorno = clientes[0]
        else:
            custo = self.distancia(
                clientes[1])  # Toma como menor distância inicial a distância do depósito até o segundo cliente listado
            retorno = clientes[1]
    ...
    def __init__(self, caminho):
        ...
        for linha in linhas:
            ...
            if cont == self._linha_capacidade:
                ...
                self._num_veiculos = int(linha[2:4].strip())
